  ...
  const content = (
    <>
      ...
      <div className="shrink-0 text-right">
        ...
        {inv.pdfUrl && (
          <span className="text-[0.65rem] text-a-accent">Ver PDF →</span>
        )}
      </div>
    </>
  );

  const className = "flex items-start gap-3 px-4 py-3.5 rounded-2xl bg-card border border-border active:bg-card-hover active:scale-[0.99] transition-all";

  if (inv.pdfUrl) {
    return (
      <a href={inv.pdfUrl} target="_blank" rel="noopener" className={className}>
        {content}
      </a>
    );
  }
