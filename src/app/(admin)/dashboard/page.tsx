          ...
          <div className="divide-y divide-border">
            {recentPosts.length === 0 && (
              <div className="flex flex-col items-center justify-center py-12 text-center">
                <div className="mb-3 flex h-10 w-10 items-center justify-center rounded-full bg-a-accent-dim">
          ...
          <div className="divide-y divide-border">
            {recentContacts.length === 0 && (
              <div className="flex flex-col items-center justify-center py-12 text-center">
                ...
                <p className="text-sm text-muted">No hay mensajes todavia</p>
              </div>
            )}
...
function StatCard({ label, value, sub, href, accent, icon }: {
  label: string; value: number; sub: string; href: string; accent?: boolean; icon: React.ReactNode;
}) {
  return (
    <Link href={href} className="admin-card admin-card-interactive p-5 block group">
      ...
      <p className={`text-3xl font-bold tracking-tight ${accent ? "text-warning" : ""}`}>{value}</p>
      <p className="mt-1.5 text-xs text-muted">{sub}</p>
    </Link>
  );
}

const POST_STATUS: Record<string, { text: string; dot: string; bg: string }> = {
...
function StatusBadge({ status }: { status: string }) {
  ...
  return (
    <span className={`ml-3 inline-flex shrink-0 items-center gap-1.5 rounded-full px-2.5 py-1 text-[0.65rem] font-medium ${badge.bg}`}>
...
const CONTACT_STATUS: Record<string, { text: string; dot: string; bg: string }> = {
